...
十条失败路径都验证过真的会失败。原有五条：削弱校验器、引入随机数、把热力档位
改回死区、catalog 声明不存在的实现、色板明度偏离阶梯。新增五条：把原语的颜色
默认参数写回函数签名（切色板后会漏出 mono 灰）、撤掉 R1 的负值守卫、撤掉空数据
守卫、把标签抽稀退回按字符数估宽、把灰度判定退回只比文档里相邻的两个色。
...
文字是否误用填充档灰 · 有无光栅图 / 动画 · 字体栈是否有 CJK 兜底 ·
